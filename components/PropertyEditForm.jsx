...
const PropertyEditForm = () => {
	...
	const handleChangeValue = (e) => {
		...
		const shouldParseInt = ['beds', 'baths', 'square_feet'].includes(name);
		const valueToSet = shouldParseInt ? parseInt(value) : value;
		if (name.includes('.')) {
	...
	const handleAmenitiesChange = (e) => {
		...
		const updatedAmenites = [...property.amenities];
	...
	useEffect(() => {
		const fetchPropertyToEdit = async (propertyId) => {
			try {
				const fetchedProperty = await fetchProperty(propertyId);
				if (fetchedProperty) {
					setProperty(fetchedProperty);
				} else {
					toast.error(`Fail to fetch property ${propertyId}`);
				}
			} catch (error) {
				toast.error(error);
			} finally {
		...
		property === null && fetchPropertyToEdit(id);
	}, []);
		...
		<>
			...
			{!loading && property && (
				<form onSubmit={handleSubmit}>
					...
					<div className='mb-4 bg-emerald-50 p-4'>
						<label className='block text-gray-700 font-bold mb-2'>
							Location
						</label>
						<input
						...
						<div className='grid grid-cols-2 md:grid-cols-3 gap-2'>
							<div>
								<input
									...
									value='Wifi'
									className='mr-2'
									checked={property.amenities.includes(
										'Wifi'
									)}
